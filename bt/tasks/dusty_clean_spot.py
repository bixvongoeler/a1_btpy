...
class DustyCleanSpot(btl.Task):
    ...
    def run(self, blackboard: btl.Blackboard) -> btl.ResultEnum:
        ...
        spot_pos = blackboard.get_in_environment(DUSTY_SPOT_CLEANING_POSITION, None)
        ...
        # Get spot parameters
        spot_radius = blackboard.get_in_environment(DUSTY_SPOT_RADIUS, None)
        current_radius = blackboard.get_in_environment(DUSTY_SPOT_CURRENT_RADIUS, None)
        current_angle = blackboard.get_in_environment(DUSTY_SPOT_CURRENT_ANGLE, None)
        ...
        if dx == 0 and dy == 0:
            ...
            blackboard.set_in_environment(DUSTY_SPOT_CURRENT_ANGLE, current_angle)
            if current_radius <= spot_radius + 1:
                ...
                blackboard.set_in_environment(DUSTY_SPOT_CURRENT_RADIUS, current_radius)
            target_pos = self.get_target_position(spot_pos, current_angle, current_radius, room_dimensions, robot_radius)
            dx, dy = self.get_next_move(robot_pos, target_pos)
